...
export function HeroSection({
  ...
  onButtonClick,
  colors = ["#1a1a1a", "#2a2a2a", "#3a3a3a", "#4a4a4a", "#5a5a5a", "#ffffff"],
  distortion = 0.8,
  swirl = 0.6,
  speed = 0.42,
  offsetX = 0.08,
  ...
  return (
    <section className={`relative w-full min-h-screen overflow-hidden bg-black flex items-center justify-center ${className}`}>
      ...
      <div className={`relative z-10 ${maxWidth} mx-auto px-6 w-full`}>
        <div className="text-center">
          ...
          >
            {title} <span className="text-white">{highlightText}</span>
          </h1>
          <p className={`text-lg sm:text-xl text-white/80 text-pretty max-w-2xl mx-auto leading-relaxed mb-10 px-4 ${descriptionClassName}`}>
            {description}
          </p>
        </div>
      </div>
